...
cover_tab_ui <- tagList(
  tags$head(
    ...
    tags$style(HTML("
      ...
      .hero-sub    { font-size:25px;font-weight:500;margin:14px 0 20px; }
      ...
      fluidRow(
        ...
        column(
          ...
          div(
            ...
            h3(HTML("<b>C</b>h<b>RO</b>matin and <b>N</b>euro<b>D</b>evelopmental disorder-related genes <b>EX</b>ploratory platform"),
              class = "hero-sub"
            ),
  ...
  div(
    ...
    div(
      ...
      div(
        class = "card-lg card-green", br(), br(),
...
input_list_diseases <- data.frame(
  TEXT = sub("^\\?", "", all_diseases$disease_name),
  SUBTEXT = all_diseases$disease_id
)
input_list_diseases <- input_list_diseases[order(input_list_diseases$TEXT), ]

input_list_diseases_CHOICES <- as.list(input_list_diseases$SUBTEXT)
names(input_list_diseases_CHOICES) <- input_list_diseases$TEXT
...
input_list_gene_ontology_subontology <- list(
...
single_gene_query_tab <- tabItem(
  tabName = "single_gene_query_tab",
  fluidRow(
    column(
      12,
      shinycssloaders::withSpinner(
...
criteria_based_search_tab <- tabItem(
  tabName = "criteria_based_search_tab",
  fluidRow(
    column(
      12,
      shinycssloaders::withSpinner(
