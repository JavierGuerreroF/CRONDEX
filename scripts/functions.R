...
jaccard_comparison_ui_generator <- function(gene_1, gene_2) {
  ...
  if (phenotypes_boolean_2 & phenotypes_boolean_1) {
    ...
    if (F) {} else {
      ...
      jaccard_distribution_plot <- violin_box_with_point(
        data_vec = data_vec,
        point = jaccard_index,
        title = NULL,
        subtitle = NULL,
...
comparison_ui_generator_CATEGORICAL_genes <- function(data_df, genes_color) {
  ...
  if (!(hp_logical | go_logical | hsa_logical)) {
    ...
    DT_table <- renderDT(server = FALSE, {
    ...
    new_order <- c(
      cols[1:2], # primeras dos columnas
      tail(cols, 1), # la última columna
      cols[3:(length(cols) - 1)]
    ) # el resto
...
compare_cellular_expression <- function(data_df, genes_color) {
  ...
  df <- data_df
  if (any(grepl("astro", colnames(df)))) {
    datatype <- "cellular"
  } else {
    datatype <- "spatial"
  }
  ...
  numerical_plot <- ggplot(df, aes(x = Sample, y = mean_expression, group = Gene, color = factor(Gene))) +
    ...
    theme(
      ...
      legend.box = "horizontal",
      legend.margin = margin(t = 0, r = 0, b = 10, l = 0),
      plot.margin = margin(t = 10, r = 10, b = 100, l = 150, unit = "pt")
    ) +
    labs(
      x = "Cell Type",
      y = "Mean Expression",
  ...
  output$dwnld_cellular_plot_genes <- downloadHandler(
    filename = function() {
      paste("cellular_expression_comparison", Sys.Date(), ".png", sep = "")
    },
    content = function(file) {
      ggsave(file, plot = numerical_plot, device = "png", width = 20, height = 12)
    }
  )
    ...
    fluidRow(
      column(
        ...
        fluidRow(
          ...
          column(
            12,
            downloadButton("dwnld_cellular_plot_genes", "Download Plot")
          )
        ),
        div(
          class = "scroll-container",
...
compare_brain_tissue_expression <- function(data_df, genes_color) {
  ...
  output$dwnld_tisular_plot_genes <- downloadHandler(
    filename = function() {
      paste("brain_tissue_expression_comparison", Sys.Date(), ".png", sep = "")
    },
    content = function(file) {
      ggsave(file, plot = numerical_plot, device = "png", width = 26, height = 9)
    }
  )
    ...
    fluidRow(
      column(
        ...
        fluidRow(
          ...
          column(
            12,
            downloadButton("dwnld_tisular_plot_genes", "Download Plot")
          )
        ),
        div(
          class = "scroll-container",
...
create_component_plot <- function(a, b,
  ...
  ## ---- datos ------------------------------------------------------------
  df <- data.frame(
    component = factor(labels, levels = labels),
    value = c(a, b)
  )
  df$xmin <- c(0, head(cumsum(df$value), -1))
  df$xmax <- cumsum(df$value)
  ...
  ggplot(df) +
    ...
    labs(
      title = paste("Similarity =", formatC(total, format = "f", digits = 2)),
      x = "Similarity", y = ""
    ) +
    theme_minimal(base_size = base_size) +
    theme(
...
plot_euler <- function(sets_list,
  ...
  on.exit(
    {
      ...
      message(sprintf(
        "\033[32mFunction 'plot_euler' ran in %.2f secs\033[0m",
        elapsed
      ))
    },
    add = TRUE
  ...
  if (max_cells > .Machine$integer.max) {
    warning(
      "Presence matrix has ", format(max_cells, big.mark = ","),
  ...
  result <- tryCatch(
    ...
    error = function(e) {
      ...
      plot.new()
      title(
        main = "Euler diagram unavailable",
...
filtrar_genes <- function(lista_genes,
  ...
  index <- sapply(lista_genes, function(gen) {
    if (is.null(gen$ncbi_gene_id) || length(gen$ncbi_gene_id) == 0) {
      test_gene_id <- NULL
    } else {
      test_gene_id <- any(gen$ncbi_gene_id %in% gene_filter)
    }
    if (is.null(gen$source) || length(gen$source) == 0) {
      test_source <- NULL
    } else {
      test_source <- any(gen$source %in% source_filter)
    }
    if (is.null(gen$phenotypes_id) || length(gen$phenotypes_id) == 0) {
      test_phenotype_id <- NULL
    } else {
      test_phenotype_id <- any(gen$phenotypes_id %in% phenotype_filter)
    }
    if (is.null(gen$diseases_id) || length(gen$diseases_id) == 0) {
      test_disease_id <- NULL
    } else {
      test_disease_id <- any(gen$diseases_id %in% disease_filter)
    }

    if (is.null(gen$complexes) || length(gen$complexes) == 0) {
      test_complex <- NULL
    } else {
      test_complex <- any(gen$complexes %in% complex_filter)
    }
    if (is.null(gen$modifications) || length(gen$modifications) == 0) {
      test_modification <- NULL
    } else {
      test_modification <- any(gen$modifications %in% modification_filter)
    }

    if (is.null(gen$gene_ontology_id) || length(gen$gene_ontology_id) == 0) {
      test_gene_ontology_id <- NULL
    } else {
      test_gene_ontology_id <- any(gen$gene_ontology_id %in% gene_ontology_filter)
    }
    if (is.null(gen$go_subontology) || length(gen$go_subontology) == 0) {
      test_go_subontology <- NULL
    } else {
      test_go_subontology <- any(gen$go_subontology %in% gene_ontology_subontology_filter)
    }
    if (is.null(gen$kegg_pathways_id) || length(gen$kegg_pathways_id) == 0) {
      test_pathway <- NULL
    } else {
      test_pathway <- any(gen$kegg_pathways_id %in% pathway_filter)
    }
...
violin_box_with_point <- function(data_vec,
  ...
  ggplot(df, aes(category, value)) +
    ...
    geom_boxplot(
      width = .2, fill = violin_color, colour = "black",
      alpha = .6, outlier.shape = NA
    ) +
    geom_point(
      data = pt_df, aes(category, point),
      colour = "red", size = 3
    ) +
    geom_label(
      data = pt_df,
