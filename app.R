## NDD PROTEINS DB APP ANTIGRAVITY
...
ui_dash <- dashboardPage(
  ...
  ")),
    ...
    #
    #     html = tagList(
    #       # 🌟 CRONDEX Logo with Fade-in Animation
    #
    #
    #       tags$div(
    #         id = "logoContainer",
    ...
    #     # tags$br(),
    #     #
    #     # # 🟡 Mensaje principal (sin animación y sin espacio entre CRONDEX y EX)
    #     # tags$div(
    #     #   id = "welcomeMessage",
    #     #   style = "
...
.main-header .navbar {
    ...
    tags$footer(
      ...
      tags$span(
        class = "footer-icons",
        tags$a(
          href = "https://github.com/yourgithub", target = "_blank",
          tags$i(class = "fab fa-github")
        ),
        tags$a(
          href = "https://www.linkedin.com/in/yourlinkedin", target = "_blank",
          tags$i(class = "fab fa-linkedin")
        ),
        tags$a(
          href = "mailto:your-email@example.com", target = "_blank",
...
server <- function(input, output, session) {
    ...
    box(
      ...
      fluidRow(
        ...
        column(
          ...
          div(
            ...
            # h3(strong("ChROmatin and NeuroDevelopmental disorder-related genes EXploratory platform")),
            h3(HTML("<strong><b>C</b>h<b>RO</b>matin and <b>N</b>euro<b>D</b>evelopmental disorder-related genes <b>EX</b>ploratory platform</strong>")),
            p(
              style = "font-size: 1.2em;",
              "CRONDEX is a specialized web portal integrating proteins associated with chromatin and their role in neurodevelopmental disorders.
      ...
      fluidRow(
        column(
          12,
          div(
            style = "background: #eef4f7; padding: 20px; border-radius: 10px; font-size: 1.1em;",
            h2("🔬 What information is available for each gene in CROND?"),
            p("Each entry in CRONDEX represents a gene and includes the following key information:"),
            tags$ul(
      ...
      fluidRow(
        column(
          12,
          div(
            style = "background: #f9f9f9; padding: 20px; border-radius: 10px; font-size: 1.1em;",
            h2("📌 How to Explore the Database?"),
            p("CRONDEX is divided into several sections to facilitate gene analysis. Below is a breakdown of each tab:"),
  ...
  # Control sidebar toggle visibility based on active tab
  observeEvent(input$tabs, {
    # Tabs where the sidebar toggle should be VISIBLE
    show_tabs <- c("single_gene_query_tab", "criteria_based_search_tab")

    if (input$tabs %in% show_tabs) {
      # Show the toggle button
      shinyjs::runjs("$('.sidebar-toggle').show();")
    } else {
      # Hide the toggle button
      shinyjs::runjs("$('.sidebar-toggle').hide();")
    }
  })
  ...
  observeEvent(input$reset_inputs, {
    ...
    picker_ids <- c(
      "gene_selection", "phenotype_selection", "disease_selection",
  ...
  observeEvent(
    ...
    },
    {
      ...
      if (selected_tab == "criteria_based_search_tab") {
        filters$gene_filter <- c()
      }
      ...
      if (all(is.null(filters))) {
        ...
        vals$gene_database_filtered <- filtrar_genes(
          genes_database,
      ...
      if (vals$database_size == 1) {
        ...
        tables$table_phenotypes <- if (is.null(gene_information$phenotypes)) {
          data.frame()
        } else {
          gene_information$phenotypes
        }
        # gene_information$phenotypes
        tables$table_diseases <- if (is.null(gene_information$diseases)) {
          data.frame()
        } else {
          gene_information$diseases
        }
        # gene_information$diseases
        tables$table_complexes <- if (is.null(gene_information$complexes)) {
          data.frame()
        } else {
          data.frame(Complexes = gene_information$complexes)
        }
        # gene_information$complexes
        tables$table_modifications <- if (is.null(gene_information$modifications)) {
          data.frame()
        } else {
          data.frame(Modifications = gene_information$modifications)
        }
        # gene_information$modifications
        tables$table_diseases_HPO <- if (is.null(gene_information$diseases_HPO)) {
          data.frame()
        } else {
          gene_information$diseases_HPO
        }
        # gene_information$diseases_HPO
        tables$table_gene_ontology <- if (is.null(gene_information$gene_ontology)) {
          data.frame()
        } else {
          gene_information$gene_ontology
        }
        # gene_information$gene_ontology
        tables$table_kegg_pathways <- if (is.null(gene_information$kegg_pathways)) {
          data.frame()
        } else {
          gene_information$kegg_pathways
        }
      ...
      vals$selected_proteins_list_text_variants <- if (vals$database_size == 0 || is.null(vals$database_size)) {
        ...
        tagList(
          ...
          actionLink("see_more_genes_btn", "See more", style = "font-weight: bold; color: #337ab7; cursor: pointer;")
        )
        # paste0(paste0(head(vals$proteins_list, 10),collapse=", "), "...")
      } else if (vals$database_size == 1) {
      ...
      observeEvent(input$see_more_genes_btn, {
        showModal(modalDialog(
          title = paste0("Selected Genes (", vals$database_size, ")"),
          div(
            style = "max-height: 400px; overflow-y: auto; word-wrap: break-word;",
            paste(vals$proteins_list, collapse = ", ")
          ),
          size = "m",
          easyClose = TRUE,
      ...
      if (vals$database_size == 1) {
        ...
        output$download_cellular_plot <- downloadHandler(
          filename = function() {
            paste("cellular_expression_plot", ".png", sep = "")
          },
          content = function(file) {
            ggsave(file, plot = plots$cellular_expression_plot, device = "png", width = 20, height = 12)
          }
        )
        ...
        if (nrow(df_sorted) > 0) {
          tissue_expression_plot <- ggplot(
            ...
            theme(
              ...
              plot.margin = margin(t = 10, r = 20, b = 100, l = 150, unit = "pt")
            )
        } else {
        ...
        output$tissue_expression_plot <- renderPlotly({
          ggplotly(plots$tissue_expression_plot)
        })

        output$download_tissue_plot <- downloadHandler(
          filename = function() {
            paste("tissue_expression_plot", ".png", sep = "")
          },
          content = function(file) {
            ggsave(file, plot = plots$tissue_expression_plot, device = "png", width = 20, height = 12)
          }
        )

        output$download_cellular_plot_2 <- downloadHandler(
          filename = function() {
            paste("cellular_expression_plot", ".png", sep = "")
          },
          content = function(file) {
        ...
        output$download_cellular_plot_3 <- downloadHandler(
          filename = function() {
            paste("cellular_expression_plot", ".png", sep = "")
          },
          content = function(file) {
        ...
        output$download_tissue_plot_2 <- downloadHandler(
          filename = function() {
            paste("tissue_expression_plot", ".png", sep = "")
          },
          content = function(file) {
        ...
        output$download_tissue_plot_3 <- downloadHandler(
          filename = function() {
            paste("tissue_expression_plot", ".png", sep = "")
          },
          content = function(file) {
      ...
      observe({
        ...
        vals$total_intersection <- Reduce(
          intersect,
      ...
      if (length(vals$all_sets) > 1 && length(vals$all_sets) < vals$max_sets) {
        ...
        euler_plot <- withTimeout(
          ...
          timeout = MAX_SECONDS,
          onTimeout = "silent"
        ) # "silent" evita error en pantalla
  ...
  observe({
        ...
        box(
          ...
          fluidRow(
            column(
              12,
              div(
                style = "background: #f9f9f9; padding: 20px; border-radius: 10px; font-size: 1.2em;",
                h3("📖 How to Start Your Search"),
                p("To explore the CRONDEX database, follow these steps:"),
                tags$ul(
          ...
          fluidRow(
            column(
              ...
              box(
                ...
                title = tagList(
                  "Cellular expression",
                  downloadButton("download_cellular_plot_2", "Download PNG", class = "btn-default btn-xs", style = "margin-bottom: 5px;margin-left: 10px;margin-right: 10px;"),
                  HTML('<a href="https://brainrnaseq.org/" target="_blank" style="color: #337ab7; font-size: 0.8em;">Go to Brain RNA-Seq </a>')
                ),
                collapsible = T,
                collapsed = T,
                solidHeader = T,
            ...
            column(
              ...
              box(
                ...
                title = tagList(
                  ...
                  downloadButton("download_tissue_plot_2", "Download PNG", class = "btn-default btn-xs", style = "margin-bottom: 5px;margin-left: 10px;margin-right: 10px;"),
                  HTML('<a href="https://human.brain-map.org/static/download" target="_blank" style="color: #337ab7; font-size: 0.8em;">Go to Allen Brain Atlas</a>')
                ),
                collapsible = T,
                collapsed = T,
                solidHeader = T,
    ...
    output$single_gene_query_info <- renderUI({
      single_gene_query_info
    })
  })
  ...
  observe({
    ...
    if (input$search_mode_button == "union") {
            ...
            fluidRow(
              column(
                ...
                box(
                  ...
                  title = tagList(
                    "Cellular expression",
                    downloadButton("download_cellular_plot", "Download PNG", class = "btn-default btn-xs", style = "margin-bottom: 5px;margin-left: 10px;margin-right: 10px;"),
                    HTML('<a href="https://brainrnaseq.org/" target="_blank" style="color: #337ab7; font-size: 0.8em;">Go to Brain RNA-Seq </a>')
                  ),
                  collapsible = T,
                  collapsed = T,
            ...
            fluidRow(
              column(
                ...
                box(
                  ...
                  title = tagList(
                    "Brain tissue expression",
                    downloadButton("download_tissue_plot", "Download PNG", class = "btn-default btn-xs", style = "margin-bottom: 5px;margin-left: 10px;margin-right: 10px;"),
                    HTML('<a href="https://human.brain-map.org/static/download" target="_blank" style="color: #337ab7; font-size: 0.8em;">Go to Allen Brain Atlas</a>')
                  ),
                  collapsible = T,
                  collapsed = T,
      ...
      result_criteria_based_search_ui <- tagList(
        ...
        if (vals$database_size > 1) {
          # MULTIPLE PROTEINS
          box(
            title = "Proteins in intersection",
            width = 12,
            solidHeader = FALSE,
            collapsible = FALSE,
                 ...
                 gap: 18px; max-width: 900px; margin: 0 auto;",
                  div(
                   ...
                   padding: 16px 18px; border: 1px solid #d5f5e3;",
                    ...
                    span(
                      "Visualize intersections between multiple annotation sets ",
    ...
    output$criteria_based_search_info <- renderUI({
      criteria_based_search_info
    })
  })
  ...
  observeEvent(input$activeTab, ignoreInit = TRUE, {
    ...
    empty_filters <- is.null(vals$filters) || # 1. la lista completa es NULL
      ...
      all(vapply(
        vals$filters, # 3. todos los elementos…
          ...
          fluidRow(
            column(
              ...
              box(
                ...
                title = tagList(
                  "Cellular expression",
                  downloadButton("download_cellular_plot_3", "Download PNG", class = "btn-default btn-xs", style = "margin-bottom: 5px;margin-left: 10px;margin-right: 10px;"),
                  HTML('<a href="https://brainrnaseq.org/" target="_blank" style="color: #337ab7; font-size: 0.8em;">Go to Brain RNA-Seq </a>')
                ),
                collapsible = T,
                collapsed = T,
          ...
          fluidRow(
            column(
              ...
              box(
                ...
                title = tagList(
                  "Brain tissue expression",
                  downloadButton("download_tissue_plot_3", "Download PNG", class = "btn-default btn-xs", style = "margin-bottom: 5px;margin-left: 10px;margin-right: 10px;"),
                  HTML('<a href="https://human.brain-map.org/static/download" target="_blank" style="color: #337ab7; font-size: 0.8em;">Go to Allen Brain Atlas</a>')
                ),
                collapsible = T,
                collapsed = T,
        ...
        if (is.null(filtered_edges) || nrow(filtered_edges) == 0) {
          ...
          if(edges.edges.length > 0){
            ...
            if (network_physics == F) {
              ...
              network_to_display <- network_to_display %>% visPhysics(
                solver = "repulsion", # 🔹 Usa un modelo de repulsión en vez de fuerza
                stabilization = list(
                  enabled = TRUE,
                  iterations = 500
                ), # Más estabilidad
  ...
  observe({
    ...
    if (length(gene_database_filtered) == length(genes_database)) {
      ...
      names(disease_subset_selection_CHOICES) <- sub("^\\?", "", disease_subset_selection_df$disease_name)
      disease_subset_selection_CHOICES <- disease_subset_selection_CHOICES[order(names(disease_subset_selection_CHOICES))]
      ...
      gene_ontology_subontology_subset_selection_CHOICES <- list(
                ...
                column(
                  ...
                  column(
                    ...
                    left_picker_input("gene_ontology_subset_selection", "Gene Ontology", gene_ontology_subset_selection_CHOICES,
                      gene_ontology_subset_selection_SUBTEXT,
                      style = gene_ontology_subset_selection_style
                    )
                  ),
  ...
  observeEvent(input$upset_plot_interactive_click, ignoreNULL = TRUE, ignoreInit = T, {
    ...
    output$genes_in_intersection_table <- renderDataTable(server = FALSE, {
      datatable_custom(genes_in_intersection_table)
    })
    # cat en verde
  ...
  observeEvent(input$filter_variants, {
    ...
    if (input$show_all_variants == T) {
      ...
      if (!is.null(pathogenicity_filter)) {
        clinvar_variants_filtered_general <- clinvar_variants_filtered_general[clinvar_variants_filtered_general$variant_pathogenicity %in% pathogenicity_filter, ]
      }
      if (!is.null(review_filter)) {
        clinvar_variants_filtered_general <- clinvar_variants_filtered_general[clinvar_variants_filtered_general$variant_review %in% review_filter, ]
      }
      if (!is.null(type_filter)) {
        clinvar_variants_filtered_general <- clinvar_variants_filtered_general[clinvar_variants_filtered_general$variant_type %in% type_filter, ]
      }
    ...
    if (!is.null(tables$clinvar_variants_filtered) || nrow(tables$clinvar_variants_filtered) > 0) {
      ...
      output$pathogenicity_freq <- renderDataTable(server = FALSE, {
        datatable_custom(pathogenicity_freq)
      })
      output$review_freq <- renderDataTable(server = FALSE, {
        datatable_custom(review_freq)
      })
      output$type_freq <- renderDataTable(server = FALSE, {
        datatable_custom(type_freq)
      })
      ...
      output$pie_chart_pathogenity <- renderPlot({
        pie_chart_pathogenity
      })
      output$pie_chart_review <- renderPlot({
        pie_chart_review
      })
      output$pie_chart_type <- renderPlot({
        pie_chart_type
      })
    ...
    if (!is.null(tables$clinvar_variants_filtered) || nrow(tables$clinvar_variants_filtered) > 1) {
      ...
      if (nrow(df_summary) > 0) {
        ...
        show_density <- if (is.null(input$show_density)) {
          F
        } else {
          input$show_density
        }
  ...
  observeEvent(input$reload_scatter_plot, {
    ...
    max_x <- if (is.null(input$max_x)) {
      vals$max_bening
    } else {
      input$max_x
    }
    max_y <- if (is.null(input$max_y)) {
      vals$max_pathogenic
    } else {
      input$max_y
    }
  ...
  observe({
    ...
    observeEvent(vals$network_data_to_DT, {
      ...
      if (is.null(vals$network_columns_selected)) {
        vals$network_columns_selected <- vals$network_columns
      }
    ...
    output$network_ui_ouput <- renderUI({
      vals$network_ui_ouput
    }) #

    output$network_threshold <- renderUI({
      vals$network_threshold
    })
  ...
  output$gpt_info <- renderUI({
    ...
    "))
              ...
              tags$h3("🧠 CRONDEX GPT Assistant", style = "margin-top: 0; color: #333; text-align: center;"),
              tags$p(
  ...
  observeEvent(input$table_help_button, { # icono “?”
    shinyalert(
      ...
      html = TRUE,
      text = div(
        style = "text-align:left;", # contenido alineado a la izquierda
        HTML("
